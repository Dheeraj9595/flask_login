...
def send_email():
    ...
    if not (subject and recipient and body):
        return (
            "Invalid request. Please provide subject, recipient, and body parameters."
        )

    msg = Message(
        subject=subject, sender="dheeraj.systango@gmail.com", recipients=[recipient]
    )
    msg.body = body
    mail.send(msg)
    ...
    if request.method == "POST":
        ...
        if file and file.filename.endswith(".xlsx"):
            ...
            try:
                ...
                db.rollback()
                flash(
                    "Error: some usres already exist or there was an issue with the data."
                )
...
def update_email_from_excel():
    ...
    try:
        ...
        if "email" not in df.columns:
            return (
                jsonify(
                    {"error": "The Excel file does not contain an 'email' column."}
                ),
                400,
            )
...
def bulk_user_create():
    try:
        ...
        return jsonify(
            {
                "message": f"{len(users)} Users saved successfully {existing_users} already registerd!!!"
            }
        )
    except IntegrityError as e:
        db.rollback()
        return (
            jsonify(
                {
                    "error": "Integrity error occurred. Likely duplicate usernames or emails.",
                    "details": str(e),
                }
            ),
            400,
        ...
        return (
            jsonify(
                {"error": "An error occurred while creating users.", "details": str(e)}
            ),
            500,
        )
...
def delete_user(user_id):
    try:
        ...
        if user is None:
            return jsonify(
                {"message": "user with user id {user_id} is deleted or not existed!!!"}
            )
        if user:
            db.delete(user)
            db.commit()
        return jsonify(
            {
                "message": f"user with user_id: {user_id} and username: {user.username}  is deleted successfully!!!"
            }
        )
    except Exception as e:
    ...
    try:
        ...
        for user in users_list:
            ...
            if user:
                ...
                db.commit()
            message.append(
                {"message": f"user with user id {user.id} is deleted successfully"}
            )
    except Exception as e:
        return jsonify({"message": f"there is a error {str(e)}"})
